...
const ScannedMealBreakdownScreen: React.FC = () => {
  ...
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList, 'ScannedMealBreakdownScreen'>>();
  ...
  const checkIfFavorite = async (): Promise<void> => {
    try {
      const isInFavorites = await FavoritesService.isFavorite(mealName, meal.restaurant?.name);
  ...
  const handleAddToLog = async (): Promise<void> => {
    ...
    try {
      ...
      const mealRequest = {
        ...
        amount: 1,
        serving_size: 'grams',
        description: `${mealName} from ${meal.restaurant?.name || 'scanned meal'}`,
      ...
      <View className="flex-1">
        ...
        >
          ...
          <View className="flex-row items-center justify-between px-5 mt-6 mb-2">
            <Text className="text-2xl font-semibold text-black">{mealName}</Text>
          ...
          <View className="mx-5 mt-2 mb-4 rounded-xl p-5 bg-[#F7F7F7]">
            <Text className="text-lg font-semibold mb-3 text-black">AI detection results</Text>
            {/* Detected Ingredients */}
            {detectedIngredients.length > 0 && (
              <View className="mb-2">
                <View className="flex-col flex-wrap">
                  {detectedIngredients.map((ingredient: string, idx: number) => (
                    <View key={ingredient + idx} className="py-1 mr-2 mb-2">
                      <Text className="text-base font-normal text-[#333333]">{ingredient}</Text>
                    </View>
                  ))}
                </View>
              </View>
          ...
          <View className="flex-row justify-between mx-5 mb-6">
            <View className="flex-1 items-center h-[75px] w-[80px] bg-[#F2F2F2F2] rounded-xl py-3 mx-1">
              <Text className="text-xs text-gloomyPurple font-medium mb-1">Protein</Text>
              <Text className="text-xl font-bold text-gloomyPurple">{macros.protein}g</Text>
            </View>
            <View className="flex-1 items-center bg-[#FEF9E6] rounded-xl py-3 mx-1">
              <Text className="text-xs text-[#FFC008] font-medium mb-1">Carbs</Text>
              <Text className="text-xl font-bold text-[#FFC008]">{macros.carbs}g</Text>
            </View>
            <View className="flex-1 items-center bg-[#FCF3FC] rounded-xl py-3 mx-1">
              <Text className="text-xs text-lavenderPink font-medium mb-1">Fat</Text>
              <Text className="text-xl font-bold text-lavenderPink">{macros.fat}g</Text>
            </View>
            <View className="flex-1 items-center bg-[#ECF5ED] rounded-xl py-3 mx-1">
              <Text className="text-xs text-kryptoniteGreen font-medium mb-1">Calories</Text>
